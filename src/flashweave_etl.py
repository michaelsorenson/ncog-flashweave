...
def flashweave_etl(sixteens_path, eighteens_path, metadata_path, sixteens_outpath, eighteens_outpath, metadata_outpath, id_map_outpath):
    ...
    metadroprows = []
    for i, sample in meta['sampleid'].items():
...
if __name__ == "__main__":
    """
    For help strings, run python flashweave_etl.py --help
    """
    parser = ArgumentParser()
    parser.add_argument("-s", "--sixteens_path", dest="sixteens_path", type=str,
                        help="path to input 16S file",
                        default="data/input/NCOG_21_16S_redo2_asv_count_tax.tsv")
    parser.add_argument("-e", "--eighteens_path", dest="eighteens_path", type=str,
                        help="path to input 18S file",
                        default="data/input/NCOG_18sV4_asv_count_tax.tsv")
    parser.add_argument("-m", "--metadata_path", dest="metadata_path", type=str,
                        help="path to input metadata file",
                        default="data/input/NCOG_sample_log_DNA_stvx_meta_2014-2020_mod.tsv")
    parser.add_argument("-S", "--output_sixteens_path", dest="output_sixteens_path", type=str,
                        help="path to output 16S file",
                        default="data/cleaned/NCOG_21_16S_redo2_asv_count_tax.tsv")
    parser.add_argument("-E", "--output_eighteens_path", dest="output_eighteens_path", type=str,
                        help="path to output 18S file",
                        default="data/cleaned/NCOG_18sV4_asv_count_tax.tsv")
    parser.add_argument("-M", "--output_metadata_path", dest="output_metadata_path", type=str,
                        help="path to output metadata file",
                        default="data/cleaned/NCOG_sample_log_DNA_stvx_meta_2014-2020_mod.tsv")
    parser.add_argument("-i", "--output_idmap_path", dest="output_idmap_path", type=str,
                        help="path to output id map, to map feature ID's to graph otu ID's",
                        default="data/cleaned/NCOG_V4_id_map.tsv")
    args = parser.parse_args()

    flashweave_etl(args['sixteens_path'], args['eighteens_path'], args['metadata_path'],
                   args['output_sixteens_path'], args['output_eighteens_path'], args['output_metadata_path'],
                   args['output_idmap_path'])
